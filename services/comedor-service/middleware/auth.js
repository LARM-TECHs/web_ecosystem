// comedor-service/middleware/auth.js
...
export const authenticateService = (req, res, next) => {
    ...
    if (!userId || !userCorreo || !userRol) {
        ...
        console.warn('Authentication Warning: Request to comedor-service missing expected user headers from API Gateway.');
...
export const authorizeRoles = (allowedRoles) => {
    return (req, res, next) => {
        ...
        if (!req.user || !req.user.rol) {
            ...
            console.warn('Authorization Warning: User information (role) not available for authorization check in comedor-service.');
        ...
        if (allowedRoles.includes(userRol)) {
            ...
            console.warn(`Authorization Warning: User with role '${userRol}' attempted to access a route requiring roles: ${allowedRoles.join(', ')} in comedor-service.`);
