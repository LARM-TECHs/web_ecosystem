...
/**
 * Inicializa y configura la conexión a la base de datos para el microservicio de comedor.
...
const sequelize = new Sequelize(
    ...
    process.env.DB_PASSWORD,
    {
        ...
        define: {
            schema: process.env.DB_SCHEMA || 'comedor', // Esquema específico para este servicio
...
export const connectDB = async () => {
    try {
        ...
        console.log('✅ Conexión a la base de datos establecida exitosamente para comedor-service.');

        const schema = process.env.DB_SCHEMA || 'comedor';
        // Crear el esquema si no existe.
        await sequelize.query(`CREATE SCHEMA IF NOT EXISTS "${schema}";`);
        console.log(`✅ Esquema '${schema}' asegurado/creado.`);

    } catch (error) {
        console.error('❌ Error al conectar o asegurar el esquema en la base de datos para comedor-service:', error);
...
export const syncModels = async () => {
    try {
        ...
        console.error('❌ Error al sincronizar los modelos de la base de datos para comedor-service:', error);
        process.exit(1);
    }
};
